...
async function triggerAiAutopilot(
  ...
  try {
    ...
    const mediaLibrary: any[] = agentConfig.mediaLibrary || [];
...
export const sendWhatsappMessage = onCall({
  ...
  const msgTypeOut = templateName ? 'template' : (mediaUrl ? (isMsgImage ? 'image' : isMsgAudio ? 'audio' : 'document') : 'text');
...
export const verifyPaypalOrder = onCall({}, async (request) => {
  ...
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'Debes iniciar sesion para verificar pagos.');
  }
